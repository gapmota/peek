    ...
    public class RedeDAO
    {
        ...
        public double[] SomaDownloadUploadDeTodosOsLaboratorios(int idUsuario)
        {
            ...
            using (SqlConnection cnx = new Banco().PegarConexao())
            {
                ...
                string sql = @"SELECT SUM(VELOCIDADE_DOWNLOAD) as VELOCIDADE_DOWNLOAD, SUM(VELOCIDADE_UPLOAD) as VELOCIDADE_UPLOAD FROM PEEK_REDE R
        ...
        public List<LabConsumo> PegarLaboratoriosQueEstaoConsumindoInternet(int idUsuario)
        {
            using (SqlConnection cnx = new Banco().PegarConexao())
            {
                ...
                string sql = @"SELECT L.ID_LAB, L.NOME, L.ANDAR, L.CAPACIDADE,  SUM(VELOCIDADE_DOWNLOAD) as VELOCIDADE_DOWNLOAD, SUM(VELOCIDADE_UPLOAD) as VELOCIDADE_UPLOAD  FROM PEEK_REDE R
                                INNER JOIN PEEK_COMPUTADOR P ON R.ID_COMPUTADOR = P.ID_COMPUTADOR 
								INNER JOIN PEEK_LAB L ON L.ID_LAB = P.ID_LAB
								INNER JOIN PEEK_USUARIO U ON U.ID_USUARIO = L.ID_USUARIO 
								WHERE U.ID_USUARIO = @ID
